...
export class PortfolioTabComponent implements OnInit {
  ...
  UpdateProjects(PageIndex: number) {
    ...
    this.viewService
      .getView('portfolio-projects', [
        ['status', this.status],
        ['uid', uid],
        ['member_id', uid],
        ['page', PageIndex],
      ])
      .subscribe((projects: ProjectCardPortfolio[]) => {
        if (PageIndex == 0) {
  ...
  SaveProjectsOrder() {
    this.mainService
      .custompost(
        'maker_sort_project_api/sort',
        this.Projects.map(project => project['nid']),
      )
      .subscribe(
        data => {},
        err => {},
  ...
  getCounts() {
    ...
    this.mainService
      .custompost('maker_count_all_projects/retrieve_count_project_draft', body)
      .subscribe(
        res => {
  ...
  loadMoreVisibilty() {
    ...
    if (this.status == 370) {
      this.countProject = this.projectsCountPublic;
    }

    if (this.status == 371) {
      this.countProject = this.projectsCountPrivate;
    }
    if (this.status == 1115) {
      this.countProject = this.projectsCountDraft;
    }
