...
export class ShowcaseComponent implements OnInit {
  ...
  getShowcaseBookmark() {
    ...
    this.viewService.getView('bookmark', [args, this.page_arg]).subscribe(
      res => {
  ...
  deleteMessage(i) {
    this.flagService
      .unflag(this.showcases[i]['nid'], this.userId, 'node_bookmark')
      .subscribe(
        response => {
          this.getShowcaseBookmark();
        },
...
*/
  deleteMessages() {
    for (var i = 0; i < this.deletedArr.length; i++) {
      this.flagService
        .unflag(this.deletedArr[i], this.userId, 'node_bookmark')
        .subscribe(
          response => {
            this.getShowcaseBookmark();
          },
