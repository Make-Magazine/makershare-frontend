...
export class OrgsProjectsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.path) {
      this.nodeService
        .getIdFromUrl(this.path, 'company_profile')
        .subscribe(id => {
          this.nid = id[0];
          if (this.nid) {
  ...
  getProjects(more?: boolean) {
    if (more) this.pages++;
    this.viewServcie
      .getView('orgs-projects', [['page', this.pages], ['nid', this.nid]])
      .subscribe(data => {
        this.projects = this.projects.concat(data);
        if (this.projects.length > 0) {
          this.company_views = data[0].org_views;
          this.showloadmoreProject =
            this.projectsCount <= this.projects.length ? false : true;
  ...
  orgsProjectsCount() {
    ...
    this.mainService
      .custompost('company_profile_api/count_projects_in_orgs', body)
      .subscribe(data => {
