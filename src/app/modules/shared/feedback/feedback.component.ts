...
import { FileService, NodeService, TaxonomyService, UserService } from '../../../core/d7services';
import { TaxonomyTerm } from '../../../core/models/drupal/taxonomyTerm';
...
export class FeedbackComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.taxonomyService
      .getVocalbularyTerms(27)
      .subscribe((data: TaxonomyTerm[]) => {
        this.feedback_types = data;
      });
    //bug types
    this.taxonomyService
      .getVocalbularyTerms(26)
      .subscribe((data: TaxonomyTerm[]) => {
        this.bug_types = data;
      });
    //feature
    this.taxonomyService
      .getVocalbularyTerms(28)
      .subscribe((data: TaxonomyTerm[]) => {
        this.features = data;
      });
    this.buildform();
  ...
  open(content, e: Event) {
    this.userService.isLogedIn().subscribe(data => {
      ...
      if (data == false) {
        ...
        this.feedbackForm.controls['field_browser'].setValue(
          this.device.browserName + ' ' + this.device.browserVersion,
        );
        ...
        this.feedbackForm.controls['field_screen_size'].setValue(
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (n !== -1) {
          ...
          if (files[i].size > 1048576) {
            this.validationMessages.field_upload_screenshots.validimagesize =
              files[i].name + ' ' + 'exceeded 1MB please upload another photo';
            this.formErrors.field_upload_screenshots[
              i
            ] = this.validationMessages.field_upload_screenshots.validimagesize;
          }
          ...
          this.validationMessages.field_upload_screenshots.validateType =
            files[i].name + ' ' + 'please choose an image';
          this.formErrors.field_upload_screenshots[
            i
          ] = this.validationMessages.field_upload_screenshots.validateType;
        }
  ...
  SaveNode() {
    ...
    if (this.full_name) {
      feedback.title = this.feedbackForm.value.title =
        this.full_name + '_' + now;
    } else {
    ...
    // console.log(feedback);
    this.nodeService.createNode(this.feedback as any).subscribe(
      (NewNode: any) => {
  ...
  onSubmit(value, type) {
    if (this.CurrentType) {
      this.feedbackForm.controls['field_want_submit'].setValue(
        this.CurrentType,
      );
    }
    if (this.BugType) {
    ...
    if (this.fileArray.length != 0) {
      for (let i = 0; i < this.fileArray.length; i++) {
        this.fileArray[i].file = NodeHelper.RemoveFileTypeFromBase64(
    ...
    if (
      ...
      this.formErrors.field_upload_screenshots.length == 0 &&
      this.formErrors.field_bug_not_in_page_ == ''
    ) {
        ...
        Observable.forkJoin(tasks).subscribe(
          x => {
            this.fileArray.forEach((element, index) => {
              ...
              this.feedback.field_upload_screenshots.und.push({
                fid: file.fid,
              });
            });
          },
