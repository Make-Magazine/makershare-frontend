...
//http://localhost:8983/solr/drupal/select?indent=on&q=*:*&spellcheck.q=test&spellcheck=on&wt=json&fl=tm_item_label
  autocomplete(query: string): Observable<any> {
    ...
    return this.http.get(globals.solrPath + 'select?fq=tm_title:('+query+') OR tm_field_first_name:('+query+') OR tm_field_last_name:('+query+') &spellcheck=true&wt=json&fl=tm_title,tm_name,tm_field_first_name,tm_field_last_name').map(res => res.json()).timeout(10000);
