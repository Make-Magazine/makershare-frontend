...
export class SolrService {
  ...
  selectContent(query: string, count: number = 10, skip: number, type: string): Observable<any> {
    query = encodeURIComponent(query);
    return this.http.get(globals.solrPath + 'select?fq=ss_type:' + type + '&fq=tm_title"' + query + '"&rows=' + count + '&start=' + skip + '&indent=on&wt=json&fl=is_nid').map(res => res.json()).timeout(10000);
  }
  ...
  selectChallenges(query: string, count: number = 10): Observable<any> {
    query = encodeURIComponent(query);
    let category = query.charAt(0).toUpperCase() + query.slice(1);
    return this.http.get(globals.solrPath + 'select?fq=ss_type:challenge&fq=tm_title:"' + query + '" OR sm_field_challenge_category$name:("'+ category +'")&rows=' + count + '&indent=on&wt=json&fl=is_nid').map(res => res.json()).timeout(10000);
  }
  ...
  selectShowcases(query: string, count: number = 10): Observable<any> {
    query = encodeURIComponent(query);
    let category = query.charAt(0).toUpperCase() + query.slice(1);
    return this.http.get(globals.solrPath + 'select?fq=ss_type:showcase&fq=tm_title:"' + query + '" OR sm_field_categories$name:("'+ category + '")&rows=' + count + '&indent=on&wt=json&fl=is_nid').map(res => res.json()).timeout(10000);
  }
  ...
  selectworkshops(query: string, count: number = 10): Observable<any> {
    query = encodeURIComponent(query);
    let category = query.charAt(0).toUpperCase() + query.slice(1);
    return this.http.get(globals.solrPath + 'select?fq=ss_type:learning_sequence&fq=tm_title:"' + query + '" OR sm_field_categories$name:("'+ category + '")&rows=' + count + '&indent=on&wt=json&fl=is_nid').map(res => res.json()).timeout(10000);
  }
