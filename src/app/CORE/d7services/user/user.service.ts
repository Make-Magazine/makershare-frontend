...
export class UserService {
  ...
  auth0_authenticate(data: any): Observable<any>{
    ...
    var obs = Observable.create(observer => {
      this.mainService.get('services/session/token',true).map(res => res.text()).subscribe(token => {
         this.saveCookies(token, null, null);
         this.mainService.custompost('auth0_service/authenticate', this.userInfo).subscribe( res => {
           observer.next(res);
           observer.complete();
         });
       });
